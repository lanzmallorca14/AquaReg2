...
/* ============================================================
   ...
   This prevents the complete owner name from being placed
   inside Last Name during re-audit / re-submit.
============================================================ */

const parseOwnerNameParts = (
  fullName: string
) => {
...
export default function AquaRegNewRegistration() {
  ...
  useEffect(() => {
    ...
    let prefillData =
      location.state?.reRegisterVessel;
    ...
    if (!prefillData) {
      ...
      try {
        ...
        console.error(
          'Failed to parse reRegisterVesselData from localStorage',
          err
        );
      }
    }
    ...
    /* --------------------------------------------------------
       ...
       If separate database columns exist, use them.

       Only fall back to owner_name when the separate fields
       are unavailable.
    -------------------------------------------------------- */

    const nameParts =
      prefillData.owner_first_name ||
      prefillData.owner_last_name
    ...
    setF({
      ...
      vesselName:
        (
          prefillData.vessel_name ||
          prefillData.gear_type ||
          ''
        ).toUpperCase(),
      ...
      barangay:
        (
          prefillData.barangay ||
          ''
        ).toUpperCase(),

      phone:
        prefillData.cp_number ||
        prefillData.phone ||
        '',

      vesselType:
        prefillData.is_motorized
          ? 'motorized'
          : 'non-motorized',

      placeOfBuilt:
        (
          prefillData.place_of_built ||
          ''
        ).toUpperCase(),

      yearBuilt:
        String(
          prefillData.year_built ||
          ''
        ),
      ...
      grossTonnage:
        String(
          prefillData.tonnage_gross ||
          prefillData.grossTonnage ||
          '0.00'
        ),

      netTonnage:
        String(
          prefillData.tonnage_net ||
          prefillData.netTonnage ||
          '0.00'
        ),
      ...
      requirements: {
        ...
        barangayClearance:
          prefillData.barangay_clearance ||
          null,
        ...
        validID:
          prefillData.valid_id ||
          null,

        bfarPermit:
          prefillData.bfar_permit ||
          null,

        marinaPermit:
          prefillData.marina_permit ||
          null
      }
    });
    ...
    toast.info(
      'Re-Audit Data Pre-filled',
      {
        description:
          `Loaded existing record ID #${prefillData.id}. Please update invalid details and re-submit.`
      }
    );
  ...
  const shouldCheckDuplicateName =
    useMemo(
      () =>
        Boolean(
          f.assetCategory
  ...
  const handleRenewalSearch =
    () => {
      ...
      /* ------------------------------------------------------
         ...
         Always prefer separate name fields.
      ------------------------------------------------------ */

      const nameParts =
        existing.owner_first_name ||
      ...
      setF(prev => ({
        ...
        grossTonnage:
          String(
            existing.tonnage_gross ??
            '0.00'
          ),

        netTonnage:
          String(
            existing.tonnage_net ??
            '0.00'
          ),
        ...
        payaoVesselName:
          String(
            existing.payao_vessel_name ??
            existing.boat_owner_vessel_name ??
            ''
          ).toUpperCase(),
        ...
        requirements: {
          ...
          bfarPermit:
            existing.bfar_permit ||
            null,

          marinaPermit:
            existing.marina_permit ||
            null
        }
      }));
    ...
    async (
      ...
      if (!navigator.onLine) {
        ...
        try {
          ...
          toast.success(
            'Image saved offline. It will sync automatically.'
          );

        } catch (_error: any) {

          toast.error(
            'Failed saving image offline'
          );
        }
      ...
      try {
        ...
        const filePath =
          `documents/${s.id ?? 'new'}/${key}/${fileName}`;
  ...
  useEffect(() => {
    ...
    const checkDuplicateNameWithinCategory =
      async () => {
          ...
          if (navigator.onLine) {
            ...
            const {
              ...
              await supabase
                .from('Vessels')
                .select(
                  'id, vessel_name, gear_type, asset_category, type'
                )
                .or(
    ...
    async () => {
      ...
      try {
        ...
        /* ----------------------------------------------------
           ...
           IMPORTANT:
           Store first / middle / last separately.
           This fixes future re-submit parsing.
        ---------------------------------------------------- */
        ...
        /* ----------------------------------------------------
           FIND EXISTING ID FOR RE-AUDIT / RENEWAL
        ---------------------------------------------------- */
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`h-12 font-black uppercase text-sm pr-10 ${
                      isDuplicate &&
                      s.mode !==
                        'RE-AUDIT'
                        ? 'border-amber-500 bg-amber-50 focus-visible:ring-amber-500'
                        : ''
                    }`}
...

                  {
                    ...
                    s.mode !==
                      ...
                      <AlertTriangle
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-amber-500 animate-pulse"
                        size={20}
                      />
                    )
                  }
...

                {
                  ...
                  s.mode !==
                    ...
                    <p className="text-[9px] font-black text-amber-600 mt-2 uppercase flex items-center gap-1">
                      ...
                      This name is already recorded under
                      {' '}
                      {f.assetCategory.toUpperCase()}
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-white rounded-2xl p-5 text-center shadow-lg border border-slate-100">
                              ...
                              <p className="text-[9px] font-black uppercase text-blue-600 mb-1">
